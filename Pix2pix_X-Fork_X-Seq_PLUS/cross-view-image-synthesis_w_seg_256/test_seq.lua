...
local Bs = torch.FloatTensor(opt.batchSize,3,opt.fineSize,opt.fineSize)
...
for n=1,math.floor(opt.how_many/opt.batchSize) do
    ...
    if opt.preprocess == 'colorization' then
       ...
       input = util.deprocessL_batch(input_L)
        ...
        output = util.deprocess_batch(netG:forward(input_Bs))
        print('Both image and segmentation as inputs')
